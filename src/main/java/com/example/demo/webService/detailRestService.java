...
public class detailRestService {
	...
	public ResponseEntity<ResponseMessage> rapportArtisteOrange(@PathVariable("id") Integer id,
			@PathVariable("datedebut") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) java.util.Date datedebut,
			@PathVariable("datefin") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Date datefin,
			@PathVariable("retenue") Double retenue) throws pdfExceptionNoDataFound, pdfExceptionDateFormat {

		String message = "done !";
		try {
			if (datedebut.after(datefin)) {
				return ResponseEntity.status(HttpStatus.OK)
						.body(new ResponseMessage(new pdfExceptionDateFormat("verifiez les dates !!").getMessage()));
			}
