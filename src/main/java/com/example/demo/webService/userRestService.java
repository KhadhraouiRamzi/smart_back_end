...
public class userRestService {
	...
	public List<user> listeArtistes() {
	...
	public void newTes1(@RequestBody user p) {
		user newU = new user(p.getId(), p.getCdate(), p.getUdate(), p.getNom(), p.getPrenom(), p.getImage(),
				p.getName(), p.getType(), p.getPicByte(), p.getnArtistique(), p.getPhone(), p.getEmail(),
				p.getPassword(), p.getDate(), p.getNationnalite(), p.getCin(), p.getDatecin(), p.getContrat(),
				p.getPart(), p.getRetenu(), p.getProposition(), p.getRoles(), p.getMarketing());
	...
	public BodyBuilder uplaodImage(@RequestParam("imageFile") MultipartFile files, @PathVariable("id") Integer id)
			throws IOException {
		System.out.println("Original Image Byte Size - " + files.getBytes().length);
		user img = new user(files.getOriginalFilename(), files.getContentType(), compressBytes(files.getBytes()));
		UserRepository.savenew(files.getOriginalFilename(), compressBytes(files.getBytes()), files.getContentType(),
				id);
		return null;
	}
	...
	/*
	 * System.out.println("Original Image Byte Size - " + files.getBytes().length);
	 * System.out.println("Contenu : "+ img.getId() + img.getName()+" "+
	 * img.getPicByte()); user newU = new user(p.getId(), p.getCdate(),p.getUdate(),
	 * p.getNom(), p.getPrenom(), p.getImage(),p.getName(),p.getType(),
	 * p.getPicByte(),p.getnArtistique(),
	 * p.getPhone(),p.getEmail(),p.getPassword(),p.getDate(), p.getNationnalite(),
	 * p.getCin(), p.getDatecin(), p.getContrat(),p.getPart(), p.getRetenu(),
	...
	public List<user> listeArtis() {
		user p = new user();
		return UserRepository.GetArts(p.getId(), p.getCdate(), p.getCin(), p.getContrat(), p.getDate(), p.getDatecin(),
				p.getEmail(), p.getImage(), p.getnArtistique(), p.getNationnalite(), p.getNom(), p.getPassword(),
				p.getPhone(), p.getPrenom(), p.getProposition(), p.getUdate(), p.getMarketing(), p.getPart(),
				p.getRetenu(), p.getName(), decompressBytes(p.getPicByte()), p.getType(), p.getFiles());

		/*
		 * for (int i = 0; i < 6; i++) { user newU1 = new user(p.getId(),
		 * p.getCdate(),p.getUdate(), p.getNom(), p.getPrenom(),
		 * p.getImage(),p.getName(),p.getType(),
		 * decompressBytes(p.getPicByte()),p.getnArtistique(),
		 * p.getPhone(),p.getEmail(),p.getPassword(),p.getDate(), p.getNationnalite(),
		 * p.getCin(), p.getDatecin(), p.getContrat(),p.getPart(), p.getRetenu(),
		 * p.getProposition(),p.getRoles(),p.getMarketing()); } return newU;
		 */
	}
