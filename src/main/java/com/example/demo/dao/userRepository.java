...
//	Optional<user> findByName(String nameimg);
	...
	@Query( "SELECT u FROM user u  join u.roles r where r.name in ('ROLE_ARTISTE','ROLE_FOURNISSEUR')") // ==> JPQL, nom de
	List GetArtFour();
	//value = "SELECT u.* FROM user u " + "inner join user_roles ur on u.id = ur.user_id "
	//+ "inner join role r on r.id =ur.role_id " + "where r.name in ('ROLE_FOURNISSEUR','ROLE_ARTISTE')\n" + "")
